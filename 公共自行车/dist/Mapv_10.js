...
//2015-09-17,计数器
var countChoropleth = 1;
//2015-9-22
var img1=new Image();
img1.src="http://192.168.80.72:17001/analyticsRes/Images/point/1.png";	
//img1.src="images/img/pic1.png";	
var img2=new Image();
img2.src="http://192.168.80.72:17001/analyticsRes/Images/point/2.png";	
//img2.src="images/img/pic2.png";	
var img3=new Image();
img3.src="http://192.168.80.72:17001/analyticsRes/Images/point/3.png";	
//img3.src="images/img/pic3.png";	
var img4=new Image();
img4.src="http://192.168.80.72:17001/analyticsRes/Images/point/4.png";	
//img4.src="images/img/pic4.png";	
var img5=new Image();
img5.src="http://192.168.80.72:17001/analyticsRes/Images/point/5.png";	
//img5.src="images/img/pic5.png";	
var img6=new Image();
img6.src="http://192.168.80.72:17001/analyticsRes/Images/point/6.png";	
//img6.src="images/img/pic6.png";	
var W=16;
var H=16;
ChoroplethDrawer.prototype.drawMap = function () {
    ...
    if (dataType === 'polyline' || dataType === 'polygon') { // 画线或面
        ...
        for (var i = 0, len = data.length; i < len; i++) {
    		...
    		var X=item.px;
			var Y=item.py;
			colorTemp=this.dataRange.getColorByRange(item.count);
			if(colorTemp=="#CCCCCC"){
				ctx.drawImage(img1,X-W/2,Y-H/2,W,H);
			}else if(colorTemp=="#FFFF00"){
				ctx.drawImage(img2,X-W/2,Y-H/2,W,H);
			}else if(colorTemp=="#FF9835"){
				ctx.drawImage(img3,X-W/2,Y-H/2,W,H);
			}else if(colorTemp=="#37C83C"){
				ctx.drawImage(img4,X-W/2,Y-H/2,W,H);
			}else if(colorTemp=="#FB8B6F"){
				ctx.drawImage(img5,X-W/2,Y-H/2,W,H);
			}else if(colorTemp=="#FF0000"){
				ctx.drawImage(img6,X-W/2,Y-H/2,W,H);
			}	
        }
        /*
        var radius = this.getRadius(); 
		...
		if(countChoropleth==1){
			ctx.canvas.style.cursor = 'pointer'
			ctx.canvas.addEventListener('click', function(e){
				var p = getEventPosition(e);
                if(p){
    				var clickPoints = []; 
    				clickPoints = drawChoropleth(p,data,ctx,radius,drawOptions,dataRange);
    				if(clickPoints.length > 0){
    					//根据经纬度获取该点详情 2015-10-9
                        getPointDetailInfo(data[clickPoints[clickPoints.length-1]]);
    				}
                }
			}, false);
		}
...
*画图
*/
function drawImage(ctx,i,x,y) {
	var image = new Image();
	image.src = "/pentaho/plugin/pentaho-cdf-dd/api/resources/public/map/cde/../images/pic"+i+".png";
...
*画分级图
*/
function drawChoropleth(p,data,ctx,radius,drawOptions,dataRange){
	var who = [];
    ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    data.forEach(function(v, i){
        /*
		ctx.fillStyle =  dataRange.getColorByRange(v.count);
		ctx.beginPath();
		...
		var X=v.px;
		var Y=v.py;	
        var colorTemp=dataRange.getColorByRange(v.count);
		if(colorTemp=="#CCCCCC"){
			ctx.drawImage(img1,X-W/2,Y-H/2,W,H);
		}else if(colorTemp=="#FFFF00"){
			ctx.drawImage(img2,X-W/2,Y-H/2,W,H);
		...
		if(p.x>=X-W/2 && p.x<=X-W/2+W){
			if(p.y>=Y-H/2 && p.y<=Y-H/2+H){
				who.push(i);
			}
		}
    });
